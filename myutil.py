...
def get_graph_info(G):
    ...
    graph_info['nedges'] = nedges
    nnodes = G.number_of_nodes()
    graph_info['nnodes'] = nnodes
    average_degree = (float(sum(dict(G.degree()).values())) / float(nnodes))
    graph_info['average_degree'] = average_degree
    density = nx.classes.function.density(G)
    graph_info['density'] = density
    if (nx.is_connected(G)):
        diameter = nx.algorithms.distance_measures.diameter(G)
        graph_info['diameter'] = diameter
        radius = nx.algorithms.distance_measures.radius(G)
        graph_info['radius'] = radius
    return graph_info
